...
@section('content')
    <div class="container-mt-5" style="padding: 16px;">
        ...
        @endif
        <form action="{{ route('archivo.make') }}" method="POST" enctype="multipart/form-data">
            @csrf
        ...
        <ul class="list-disc pl-5">
            @foreach ($archivos as $archivo)
                <li>
                    <a href="{{ route('archivo.descargar', ['archivo' => basename($archivo)]) }}" class="text-blue-500 hover:underline">
                        {{ basename($archivo) }}
                    </a>
                </li>
            @endforeach
